...
def authorized_only(handler):
    """Decorator to check if user is authorized, with owner bypass"""
    async def wrapper(update: Update, context: ContextTypes.DEFAULT_TYPE):
        user = update.effective_user
        user_id = user.id
        
        # Owner bypass - owner can always use commands even if banned
        if user_id == OWNER_ID:
            await handler(update, context)
            return
        
        # For non-owner users, check if banned
