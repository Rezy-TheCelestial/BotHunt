...
🔔 *Notify Chat:* `{target_chat_id}`
                        ...
                        try:
                            bot_app = ApplicationBuilder().token(BOT_TOKEN).build()
                            await bot_app.bot.send_message(chat_id=NOTIFY_CHAT_ID, text=f"✨shiny Pokemon found in {account_name}")
...
• Time: `{datetime.now().strftime('%H:%M:%S')}`
                        ...
                        try:
                            bot_app = ApplicationBuilder().token(BOT_TOKEN).build()
                            await bot_app.bot.send_message(chat_id=NOTIFY_CHAT_ID, text=f"⚡ Daily Hunt limit reached for {account_name}")
